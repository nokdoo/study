...
struct tnode *addtree(struct tnode *, struct tnode *, char *);
...
struct tnode *addtree(struct tnode *root, struct tnode *p, char *w){
	int cond;
	...
	if(p == NULL){
		...
		p->left = addtree(root, p->left, w);
	else
		p->right = addtree(root, p->right, w);
