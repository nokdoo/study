...
이와는 대조적으로 deferred cancellation는, 한 스레드는 target thread가 취소될 것임을 나타낸다. 하지만 cancellation은 target thread가 취소되어야 하는지를 결정하는 flag를 확인한 후에만 발생한다. 스레드는 안전하게 취소될 수 있는 시점에 이 확인작업을 수행할 수 있다.
...
기본 cancellation type은 deferred cancellation이다. 여기에 cancellation은 스레드가 cancellation point에 도달할 때만 일어난다. cancellation point를 수립하기 위한 기술은 `pthread_testcancel()` 함수를 호술하는 것이다. 만일 cancellation 요청이 보류중이라면 cleanup handler로 알려짐 함수가 호출될 것이다. This function allows any resources a thread may have acquired to be released before the thread is terminated.
...
many-to-many 또는 two-level 모델은 구현하는 많은 시스템들은 사용자와 커널 스레드 사이의 중간 데이터 구조를 위치시킨다. lightweight process 또는 LWP라고 알려진 이 데이터 구조는 [Figure 4.13, p187]에 있다. user-thread library에 대해 LWP는, 어플리케이션이 사용자 스레드를 실행하도록 스케쥴링할 수 있는 virtual processor로 보인다. 각 LWP는 커널 스레드에 첨부된다. 그리고 물리적인 프로세서에서 실행하기 위해 os가 스케쥴링하는 커널 스레드이다. 커널 스레드가 차단된다면(I/O operation의 완료를 기다리는 동안) LWP 또한 차단된다. 잇따라 LWP에 첨부된 user-level thread도 역시 차단된다.

어플리케이션은 효율적으로 식행하기 위해서 많은 수의 LWP를 요구할 수 있다. single-processor에서 실행 중인 CPU-bound 어플리케이션을 생각하자. 이 이러면 한 번에 하나의 스레드만이 실행될 수 있고 하나의 LWP이면 충분하다. 하지만 I/O-intensive인 어플리케이션은 실행을 위해서 multiple LWP을 요구할 수 있단. 일반적으로 LWP는 각각이 동시에 system call을 차단하는 것이 요구된다. 예를 들어 5개의 다른 file-read 요청이 동시에 발생했다고 가정하자. 필요한 LWP가 커널에서 I/O의 완료를 기다릴 수 있기 때문에 총 5개의 LWP가 필요하다. 프로세스가 4개의 LWP만을 가지고 있다면 다섯 번째 요청은 커널로부터 LWP이 반환되기를 기다려야 한다.

user-thread library와 커널 간 통신을 위한 scheme는 scheduler activation으로 알려져 있다. 이것은 다음을 작업한다. 커널은 virtual processor(LWP)의 집합과 어플리케이션을 제공하고, 어플리케이션은 이용 가능한 virtual processor에서 사용자 스레드를 스케쥴링할 수 있다. 추가적으로 커널은 어플리케이션에 특정 이벤트에 대해 알려야 한다. 이 프로시저는 upcall로 알려져 있다. upcall은 upcall handler를 가진 thread library에 의해서 handle 된다. 그리고 upcall handler는 virtual processor 에서 실행되어야 한다. upcall을 일으키는 이벤트는 어플리케이션 스레드가 차단될즈음에 발생한다. 이런 경우 커널은, 스레드가 곧 차단되려하는 것을 알리고 특정 스레드를 식별하는 어플리케이션에 upcall을 생성한다. 그러면 커널은 새로운 virtual processor를 어플리케이션에 할당한다. 어플리케이션은 이 virtual processor에서 upcall handler를 실행시키고 스레드 차단의 상태를 저장하고 스레드 차단이 실행되는 virtual processor의 권리를 포기한다. 그러면 upcall handler 새로운 virtual processor에서 실행될 자격이 있는 다른 스레드를 스케쥴링 한다. 스레드 차단이 기다리고 있는 이벤트가 발생할 때, 커널은 이전에 차단된 스레드가 이제는 실행될 자격이 있음을 알리는 thread library에 또 다른 upcall을 생성한다. 이 이벤트를 위한 upcall handler 역시 virtual processor를 요구한다. 그리고 커널은 새로운 virtual processor를 할당하거나, 사용자 스레드 중 하나를 선점하고 자신의 virtual processor에 upcall handler를 실행한다. 어플리케이션은 차단되지 않은 스레드를 실행될 자격이 있는 것으로서 표시한 후, 이용 가능한 virtual processor에서 실행될 자격이 있는 스레드를 스케쥴링 한다.
